...
class EdgeCreator:
    ...
    def recursively_find_parent_id(
        self, json_object, from_connection, to_connection, current_parent
    ):
        ...
        if (
            json_object["type"] == "If.body"
            or json_object["type"] == "While.body"
            or json_object["type"] == "For.body"
            or json_object["type"] == "If.else"
        ):
        ...
        if "value" in json_object:
            for inner_json_object in json_object["value"]:
                self.recursively_find_parent_id(
                    inner_json_object, from_connection, to_connection, current_parent
                )
                ...
                for connection in connections:
                    if connection:
                        ...
                        for parent_id, edges in self.parents.items():
                            if (
                                edges[0]["id"] == json_object_id
                                and edges[1]["id"] == connection["id"]
                            ):
    ...
    def display_connections(self):
        for json_object_id, connections in self.connections.items():
            ...
            if "command" in json_object:
                print(
                    json_object["command"].rjust(40, " "),
                    "\tconnected to:\t",
                    connections_print_format,
                )
            else:
                print(
                    json_object["type"].rjust(40, " "),
                    "\tconnected to:\t",
                    connections_print_format,
                )
